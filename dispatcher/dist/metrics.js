...
exports.incrementRunCounters = incrementRunCounters;
exports.__resetCountersForTest = __resetCountersForTest;
...
const PRECHECK_RESULTS = ["skip", "run", "error"];
// ---------------------------------------------------------------------------
 ...
 */
function incrementRunCounters(input) {
...
function __resetCountersForTest() {
...
function renderMetrics(input) {
    ...
    out.push("# HELP selfheal_dispatcher_last_run_duration_seconds Duration of the most recent dispatch run.");
    ...
    out.push("# HELP selfheal_dispatcher_healthcheck_dep Per-dependency healthcheck result (1 healthy, 0 down/unknown).");
    out.push("# TYPE selfheal_dispatcher_healthcheck_dep gauge");
    const byDep = new Map();
    for (const r of input.lastHealthcheck?.results ?? [])
        byDep.set(r.dep, r.healthy);
    for (const dep of HEALTHCHECK_DEPS) {
        out.push(`selfheal_dispatcher_healthcheck_dep{dep="${dep}"} ${byDep.get(dep) ? 1 : 0}`);
    }
    out.push("# HELP selfheal_dispatcher_healthcheck_healthy Number of healthy dependencies in the last healthcheck.");
    ...
    out.push("# HELP selfheal_dispatcher_healthcheck_total Number of dependencies probed in the last healthcheck.");
    ...
    out.push("# HELP selfheal_dispatcher_last_precheck Most recent poll pre-check result (1 for the active result).");
